...
export class PageRepository
	...
	public static async getInstance() {
		if (!this.instance) {
			...
			this.instance = new PageRepository(orm)
		}
	...
	async createOne(
		...
		const result = await this.orm.createPage(
			userID,
			data
		)
	...
	async getAll(userID: string): Promise<TSelectPage[]> {
		...
		const result = await this.orm.getPagesByUserId(userID)
	...
	async getOne(
		...
		const result = await this.orm.getPageById(
			userID,
			id
		)
		return this.logger.logAndReturn(
	...
	async updateOne(
		...
		const result = await this.orm.updatePageById(
			userID,
			id,
			updates
		)
	...
	async deleteOne(
		...
		const result = await this.orm.deletePageById(
			userID,
			id
		);
		return this.logger.logAndReturn(
